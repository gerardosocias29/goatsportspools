...
/**
 * Reusable Squares pool creation wizard (6 steps).
 *
 * Props:
 *  - onSuccess(poolNumber, poolData)  : called after pool is created
 *  - onCancel()                        : optional — called when user cancels (step 1 back)
 *  - compact?                          : if true, renders without the outer page wrapper (for modal use)
 */
const CreatePoolForm = ({ onSuccess, onCancel, compact = false }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data?.status) {
        ...
        if (onSuccess) onSuccess(poolNumber, response.data.data);
      }
    } catch (error) {
  ...
  return (
    <div className={compact ? '' : 'space-y-6'}>
      {/* Progress Bar */}
      <div className={`rounded-2xl border border-gray-200 bg-white p-4 ${compact ? 'mb-4' : 'mb-6'} dark:border-gray-800 dark:bg-white/[0.03]`}>
      ...
      {/* Submit error */}
      {errors.submit && (
        <div className={`rounded-xl border border-error-300 bg-error-50 p-4 ${compact ? 'mb-4' : 'mb-6'} dark:border-error-500/30 dark:bg-error-500/10`}>
          <p className="text-sm font-medium text-error-600 dark:text-error-400">{errors.submit}</p>
        </div>
      )}

      {/* Form Card */}
      <div className={`rounded-2xl border border-gray-200 bg-white ${compact ? 'p-5' : 'p-6 md:p-8'} dark:border-gray-800 dark:bg-white/[0.03]`}>
